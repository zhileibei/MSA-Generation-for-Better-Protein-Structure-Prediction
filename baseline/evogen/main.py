...
from mindsponge.common.residue_constants import restypes_with_x_and_gap, MAP_HHBLITS_AATYPE_TO_OUR_AATYPE
...
parser.add_argument('--data_config', default=f"{SCRIPT_PATH}/config/data.yaml", help='data process config')
parser.add_argument('--model_config', default=f"{SCRIPT_PATH}/config/model.yaml", help='model config')
parser.add_argument('--evogen_config', default=f"{SCRIPT_PATH}/config/evogen.yaml", help='evogen config')
...
parser.add_argument('--checkpoint_path', default=f"{SCRIPT_PATH}/ckpt/MEGAEvoGen.ckpt", help='checkpoint path')
...
def evogen_augmentation(args):
    ...
    for seq_file in seq_files:
        ...
        for i in range(data_cfg.common.num_recycle):
            feat_i = [Tensor(x[i]) for x in feat]
            (prev_pos, prev_msa_first_row, prev_pair, predicted_lddt_logits), msa_vae_result, msa_feat_new = megaevogen(
                *feat_i, *fake_data, prev_pos, prev_msa_first_row, prev_pair)

        os.makedirs(f'./result/{seq_name}', exist_ok=True)

        # <<<<<<<<<<<<<<<<<<<<<<<<<<<<< Output Generated MSA
        # import pdb; pdb.set_trace();
        msa_vae_result_np = { k:v.asnumpy() for k,v in msa_vae_result.items() }
        msa_vae_result_np['msa_feat_new'] = msa_feat_new.asnumpy()
        pickle.dump(msa_vae_result_np, open(f'./result/{seq_name}/msa_gen.pkl', 'wb'))
        with open(f"./result/{seq_name}/msa.txt", 'w') as OUT:
            seq_set = set()
            for seq_encoded in raw_feature['msa']:
                seq = "".join([restypes_with_x_and_gap[MAP_HHBLITS_AATYPE_TO_OUR_AATYPE[d]] for d in seq_encoded])
                seq_set.add(seq)
                print(seq, file=OUT)
            print("", file=OUT)
            generated_msa_encoded = msa_vae_result_np['msa_logits'].argmax(2)
            for seq_encoded in generated_msa_encoded:
                seq = "".join([restypes_with_x_and_gap[d] for d in seq_encoded])[:ori_res_length]
